...
fn test_archive_not_found_display_does_not_panic_on_very_long_path() {
    ...
    let err = PackageError::ArchiveNotFound { path: long_path.clone() };
    // Must not panic; result must contain the path.
    let text = err.to_string();
    assert!(text.contains(&long_path[..64]), "display must include path prefix");
}
...
fn test_error_display_has_consistent_prefix_format() {
    ...
    let not_found = PackageError::ArchiveNotFound { path: "p".to_string() };
    let network = PackageError::NetworkFailed {
        name: "n".to_string(),
        reason: "r".to_string(),
    };
    let manifest = PackageError::ManifestParse("m".to_string());
    let not_in = PackageError::NotInManifest { name: "x".to_string() };
    ...
    for (label, text) in [
        ...
        assert!(
            !text.is_empty(),
            "{label} display must not be empty"
        );
        assert!(
            !text.starts_with(' '),
